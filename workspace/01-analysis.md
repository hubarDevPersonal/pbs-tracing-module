...
| D13 | Traces whose `exitpoint` never runs (early 4xx/5xx) | print partial / drop and keep the slot / drop and give the slot back | **drop; the slot is given back once its 5-minute lease expires unconfirmed** | A partial packet has no final response by definition. There is no hook PBS invokes on the 500 path (`auction.go` returns before `sendAuctionResponse`), so the loss cannot be detected at once; a lease detects it late but surely, and the amount then counts collected packets as the task says. PBS lets an auction run longer than the lease (`auction_timeouts_ms.max` is 10 minutes in the provided `pbs.yaml`), so a slot given back is also revoked: the old auction, if it is still alive, loses the right to write, and the amount is never exceeded (spec FR-10 AC5). See §5 item 6. |
...
6. **Slot reserved without a packet.** `Tracer.Begin` reserves the packet slot at `processed_auction_request`. If `HoldAuction`
   ...
   its window is still open. The module has no hook on that path, which is why the loss is detected by the lease and not at once.
   The lease cannot tell a failed auction from a slow one: an auction may run for up to `auction_timeouts_ms.max`, 10 minutes here.
   A slot given back is therefore revoked, and a slow auction that loses its slot writes nothing (FR-10 AC5); without that, every
   lease period could add another `TracePacketsAmount` packets.
