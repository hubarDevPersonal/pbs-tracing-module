...
# Phase A: the assessment request (item 3 soft, sample bidders answer 204). Phase B: the same request plus
# onetag's documented test publisher under the second rule — item 3 asserted strictly (see scripts/e2e-docker.sh).
#
...
log "1/7 install module into $PBS_DIR, vet, test, build"
...
log "5/7 phase B: live-bid request (onetag test publisher, partner 33415-10498) ×2 — second one must not be traced"
for i in 1 2; do
...
tail -n +"$((phase_a_lines + 1))" "$WORK/trace.ndjson" >"$WORK/trace-live.ndjson"
